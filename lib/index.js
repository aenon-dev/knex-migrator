...
KnexMigrator.prototype.reset = function reset(options) {
    ...
    if (force) {
        if (this.dbConfig.client !== 'pg') {
            ...
            this.connection = database.connect({
                client: this.dbConfig.client,
                connection: _.omit(this.dbConfig.connection, ['database'])
            });
        }
        ...
        return database.drop({
            ...
            // Database does not exist. MySql or Postgres.
    ...
    if (this.dbConfig.client === 'pg') {
        return this._pgReset()
            .then(function () {
                self.connection = database.connect({
                    client: self.dbConfig.client,
                    connection: _.omit(self.dbConfig.connection, ['database'])
                });
            })
            .then(function (){
                return database.ensureConnectionWorks(self.connection);
            })
            .then(function () {
                return database.drop({
                    connection: self.connection,
            ...
            .catch(function onRestError(err) {
                ...
                // CASE: Database does not exist. For Postgres.
                if (err.code === '3D000') {
                    return Promise.resolve();
                }
...
/**
 * @description Private helper for a full hard reset in postgres. 
 * The separation of tasks is necessary to destroy connections before calling drop.
 *
 * @param {*}
 * @returns {*}
 * @private
 */
KnexMigrator.prototype._pgReset = function _pgReset() {
    let self = this;
